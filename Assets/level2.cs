...
public class level2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.Log(m_fb.lv2bullets);
        if(countdown.seconds == 0)
        {
        if(m_fb.toonkilled >= 7)
        {
            Debug.Log("Level completed");
            completedscreen.SetActive(true);
        }
        else if(m_fb.toonkilled != 7)
        {
            Debug.Log("Game over");
...
Debug.Log("Level completed");
            ...
            else if(m_fb.lv2bullets<=0 && m_fb.toonkilled != 7)
            {
                 Debug.Log("Game over");
        ...
        if(totalbullets != m_fb.lv2bullets)
        {
          float n =  totalbullets - m_fb.lv2bullets;
          for(int i = 0 ;i < n ;i++)
          {
